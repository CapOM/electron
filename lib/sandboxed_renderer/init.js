...
const { logSecurityWarnings, shouldLogSecurityWarnings } = require('@electron/internal/renderer/security-warnings')

// Warn about security issues
window.addEventListener('load', function loadHandler () {
  if (shouldLogSecurityWarnings()) {
    const webPreferences = electron.ipcRenderer.sendSync('ELECTRON_BROWSER_GET_LAST_WEB_PREFERENCES')
